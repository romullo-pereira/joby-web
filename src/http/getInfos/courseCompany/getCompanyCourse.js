const getCompanyCourses = (id) => {
  const http = new XMLHttpRequest();

  http.open('GET', `${baseUrl}/company-course/courses-company/${id}`, true);

  http.onreadystatechange = function () {
    if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
      const response = JSON.parse(http.responseText);
      concatCompanyCourseInfos(response);
    }
  };
...
function concatCompanyCourseInfos(courses) {
